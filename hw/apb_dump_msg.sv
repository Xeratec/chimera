...
// When the target synthesis is used, the module behaves as a bypass
// for the APB messages.
// During simulation instead, the APB messages directed to the
// DUMP address 0x30004ffc are printed in a UART fashion on the console.
...
`else
  ...
  always_comb begin: gen_dump
    apb_req_o = apb_req_i;
    apb_rsp_o = apb_rsp_i;
    // Mask the APB request if targetting the dump address
